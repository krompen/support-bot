...
last_check_time = 0  # простая защита от спама
...
def generate_variations(base: str, count: int = 20) -> list:
    ...
    for _ in range(count * 4):
        ...
        new = ''.join(v)
        if 5 <= len(new) <= 32 and re.match(r'^[a-z_]+$', new):
            variations.add(new)
        
        # underscore
        if len(base) >= 5 and random.random() > 0.55:
            pos = random.randint(1, len(base)-2)
            variations.add(base[:pos] + '_' + base[pos:])
    
    valid = [v for v in variations if re.match(r'^[a-z_]{5,32}$', v)]
    random.shuffle(valid)
    return valid[:count]

def generate_letters(length: int, count: int = 15) -> list:
    vowels = "aeiou"
...
async def letters_search(call: CallbackQuery):
    ...
    await call.message.edit_text(f"🔄 Ищу свободный ник из {length} букв (только буквы)...")
...
print("✅ Бот name search bitok запущен (только буквы + защита от бана)")
async def main():
    await init_db()
